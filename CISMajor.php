...
$mmfallquery= 'SELECT course_number FROM cis_major WHERE Fall_2016 = "Yes" ';
...
$mmfallquery= 'SELECT course_number FROM cis_major WHERE Spring_2017 = "Yes" ';
$mmresultfall= mysql_query($mmfallquery) or die ("Query to get data from firsttable failed: ".mysql_error());
echo "<select name = cismfall>";
while ($mmfallrow=mysql_fetch_array($mmresultfall)){
    $mmTitlefall = $mmfallrow["course_number"];
...
$mmfallquery= 'SELECT course_number FROM cis_major WHERE Requirement = "Y" ';
$mmresultfall= mysql_query($mmfallquery) or die ("Query to get data from firsttable failed: ".mysql_error());
echo "<select name = cismfall>";
while ($mmfallrow=mysql_fetch_array($mmresultfall)){
    $mmTitlefall = $mmfallrow["course_number"];
...
$mmfallquery= 'SELECT course_number FROM cis_major WHERE elective = "Y" ';
$mmresultfall= mysql_query($mmfallquery) or die ("Query to get data from firsttable failed: ".mysql_error());
echo "<select name = cismfall>";
while ($mmfallrow=mysql_fetch_array($mmresultfall)){
    $mmTitlefall = $mmfallrow["course_number"];
...
th, td {
  ...
  echo '<form method = "post" action= "http://www.bentley.edu/files/2015/09/01/Major%3DBSCS.pdf">';
