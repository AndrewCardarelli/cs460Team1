...
$mmquery= 'SELECT course_number FROM ecofi_major';
...
$mmfallquery= 'SELECT course_number FROM ecofi_major WHERE Fall_2016 = "Yes" ';
...
$mmfallquery= 'SELECT course_number FROM ecofi_major WHERE Spring_2017 = "Yes" ';
$mmresultfall= mysql_query($mmfallquery) or die ("Query to get data from firsttable failed: ".mysql_error());
echo "<select name = cismfall>";
while ($mmfallrow=mysql_fetch_array($mmresultfall)){
    $mmTitlefall = $mmfallrow["course_number"];
...
$mmfallquery= 'SELECT course_number FROM ecofi_major WHERE Requirement = "Y" ';
$mmresultfall= mysql_query($mmfallquery) or die ("Query to get data from firsttable failed: ".mysql_error());
echo "<select name = cismfall>";
while ($mmfallrow=mysql_fetch_array($mmresultfall)){
    $mmTitlefall = $mmfallrow["course_number"];
...
$mmfallquery= 'SELECT course_number FROM ecofi_major WHERE elective = "Y" ';
$mmresultfall= mysql_query($mmfallquery) or die ("Query to get data from firsttable failed: ".mysql_error());
echo "<select name = cismfall>";
while ($mmfallrow=mysql_fetch_array($mmresultfall)){
    $mmTitlefall = $mmfallrow["course_number"];
...
function getResult(){
    $sqlform = "SELECT * FROM ecofi_major WHERE course_number LIKE '". $_POST['ecofim']."'";
...
/*
  ...
  echo '<form method = "post" action= "https://www.bentley.edu/files/2016/08/26/Major%3DBSEF%2016-17.pdf">';
